...
- 포함: 대화용 `0x82xx`와 게임 메뉴용 `0x83xx`에 배치한 247자 한글 폰트, 프리뷰 레코드 33개
  (`0002`~`0012`, `0016`~`0020`, 게임 화면 공통 문구 11개, `0058`~`0063`)
- `0002`~`0012`와 게임 화면 공통 문구 11개는 원래 슬롯에 맞춘 압축형 프리뷰이고,
  `0016`~`0020`은 자연스러운 초안이 슬롯에 맞아 삽입됨
- 길이가 넘치는 초반 원고는 `translation/relocation-plan.tsv`에 기록했으며 후속 포인터/은행 검증 대상
