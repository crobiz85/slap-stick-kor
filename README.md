...
- 버전: `v0.1.15-alpha`
- 배포 형식: IPS만 제공
- 패치: [`patches/robotrek-korean-v0.1.15-alpha.ips`](patches/robotrek-korean-v0.1.15-alpha.ips)
...
- 적용 결과 SHA-256: `81A3603F7BF9A5C5AFD8B6D3E6962D21DE4FE716F1B6EAF92779812DB0962B6B`
...
- 현재 공개 배포본은 `v0.1.15-alpha`입니다. 2026-09-04 사용자가 오프닝의 정상 출력을
  확인하고 공개를 요청한 테스트본과 동일한 IPS입니다.

## v0.1.17-alpha 로컬 테스트 변경 (미공개)
...
- 엔딩 직전 저장 파일은 확인되지 않아 실제 엔딩 전체 이벤트 진행 검증은 남아 있습니다.
  별도 렌더링 검사는 엔딩 플레이 완료를 의미하지 않습니다. 기존 저장 파일은 변경하지 않았고,
  공개 배포본은 `v0.1.15-alpha`를 유지합니다.
...
2. 사용하는 IPS 패처 또는 에뮬레이터의 IPS 기능으로
   `robotrek-korean-v0.1.15-alpha.ips`를 원본에 적용합니다.
