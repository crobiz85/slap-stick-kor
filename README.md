...
- 버전: `v0.1.17-alpha`
- 배포 형식: IPS만 제공
- 패치: [`patches/robotrek-korean-v0.1.17-alpha.ips`](patches/robotrek-korean-v0.1.17-alpha.ips)
...
- 적용 결과 SHA-256: `24F254AF637216A0E81790882E93E0D838FF91327C3A6A6D2C4EBD0AF327D7F6`
...
- 공개 업데이트(GitHub 푸시·공개 IPS 교체·릴리스)는 진행 불가·크래시 등 큰 문제를
  해결할 때를 원칙으로 합니다. 단순 번역 추가나 일정 경과만으로 자동 공개하지 않습니다.
  사용자가 테스트 후 명시적으로 공개를 요청한 경우에도 해당 버전을 공개할 수 있습니다.
...
- 현재 공개 배포본은 `v0.1.17-alpha`입니다. 2026-09-08 사용자가 엔딩의 정상 진행을
  확인하고 공개를 요청한 테스트본과 동일한 IPS입니다.

## v0.1.17-alpha 공개 변경

- 새 스크린샷 5장을 확인해 엔딩의 별도 연출용 대사 27개 화면을 번역했습니다.
  각 페이지의 고정 주소, 장면 전환·대기·화자·색상 명령과 창 설정은 보존했습니다.
- 왕자에게 말하는 짧은 대사와 최종 추격 전 나기사의 저장 안내·선택지를 번역했습니다.
  저장하지 않기를 선택하면 돌아오는 부모의 응원 문단도 `0x0CDDA9`에 고정했습니다.
- 정적 검증과 IPS 재적용 일치 검사, 별도 렌더링 테스트에서 엔딩 27개 화면의 한글 표시를
  확인했습니다. 블랙모어 재대화의 종료와 이동도 기존 슬롯 1로 재검증했습니다.
- 2026-09-08 사용자가 실제 플레이로 엔딩 전체의 정상 진행을 확인했습니다.
  기존 저장 파일은 변경하지 않았으며, v0.1.16의 블랙모어 수정도 포함합니다.

### 알려진 문제

- 일부 선택 창에서 선택지와 대사의 표시 위치가 맞지 않는 문제가 남아 있습니다.
- 전체 선택 창의 위치를 일괄 수정하는 과정에서 다른 표시 문제가 발생해 해당 변경은 원복했습니다.
  따라서 `v0.1.17-alpha`에는 선택 창의 일괄적인 위치 수정이 포함되지 않았습니다.
- 개별 선택지의 번역과 이벤트 분기 보존 검증은 일괄적인 화면 위치 수정과 별개의 작업입니다.
  모든 선택 창의 표시가 정상이라는 의미는 아닙니다.

## v0.1.16-alpha 로컬 테스트 변경 (미공개)

- 블랙모어에게 다시 말하면 화자 이름만 나오고 대화가 끝나지 않던 문제를 수정했습니다.
  재대화는 `D3` 명령으로 `0x0CC36C` 문단에 진입하지만, 번역 문단이 앞쪽으로 이동해
  기존 주소가 빈 영역이 된 것이 원인이었습니다.
- 열쇠 거래 대사를 분할해 “프로그램은 요새 컴퓨터에 있어” 문단을 원래 주소에 고정했습니다.
  앞 문단의 페이지 전환과 재대화 점프 명령은 보존했으며, 번역 문장 자체는 바꾸지 않았습니다.
- 제공된 PC 슬롯 1에서 v0.1.15의 증상을 재현하고, 수정본의 두 페이지 출력·대화 종료·이동을
  Snes9x Libretro로 확인했습니다. 고정 진입점 회귀 검사와 IPS 재적용 일치 검사도 통과했습니다.
- 사용자 에뮬레이터에서의 재확인은 남아 있습니다. 공개본은 `v0.1.15-alpha`를 유지합니다.

## v0.1.15-alpha 공개 변경 (이전 공개본 v0.1.12 대비)

로컬 테스트 버전 `v0.1.13`과 `v0.1.14`의 수정도 포함합니다. 아래에는 버전별 신규 변경을
구분해 기록합니다. 기존에 전달한 `v0.1.15`의 ROM·IPS 내용은 변경하지 않았습니다.

- 오프닝 내레이션 15개 화면을 번역했습니다. 우주와 생명체 소개부터 퀸티닉스·로코코,
  해커들의 습격, 안드로이드, 주인공의 이사와 꿈까지 포함합니다.
- 각 문구만 원래 공간 안에서 교체하고, 고정 진입 주소·창 설정·출력 속도 명령과
  `E1`/`D0`/`CC` 장면 전환 명령을 보존했습니다.
- Snes9x Libretro에서 새 부팅 후 오프닝의 한글 출력, 자동 장면 전환,
  종료 후 타이틀 복귀를 확인했습니다. 정적 검증과 IPS 재적용 일치 검증도 통과했습니다.
- 타이틀 로고와 저작권 표시는 이번 번역 범위에 포함하지 않았습니다.
  기존 저장 파일은 변경하지 않습니다.
- 2026-09-04 사용자가 오프닝의 정상 출력을 확인했습니다. 이 확인은 오프닝에 한정하며,
  아래 상점·선택지 등 다른 장면이나 게임 전체의 정상 진행 확인을 의미하지 않습니다.

### v0.1.14에서 먼저 반영한 수정

- 2026-09-04 추가된 스크린샷 9장의 중복 제외 7개 미번역 구간을 번역했습니다.
  두 상점의 공용 구매/구경 선택지, 연구소 안내 선택지, 쿠키 대사,
  우주선의 짧은 웃음·반응 대사, 로즈·촌장의 부탁 대화입니다.
- 같은 로즈·촌장 대화에서 이어지는 2개 구간도 번역해 총 9개 출력 구간을 추가했습니다.
- 창 크기·색상·화자·속도/대기 명령, 선택지 줄 수와 `CC`/`DE` 복귀 주소를 보존했습니다.
  정적 검증과 IPS 재적용 일치 검증을 통과했으며, 새 제보 장면의 실제 플레이 확인은 남아 있습니다.
- `v0.1.13`의 테두리 손상 수정도 포함합니다.

### v0.1.13에서 먼저 반영한 수정

- 폴론의 대사가 끝난 뒤 창 닫기 명령 `C8`과 복귀 명령 `CC` 사이에 남은 공백이
  창 테두리 타일을 덮어쓰던 문제를 수정했습니다. 공백은 닫기 전에 처리하며 복귀 주소는 유지합니다.
- 고정 길이 대사 생성 시 `C8`·`D1`·`CC` 종료 명령 뒤에 패딩이 끼지 않도록 공통 처리와 검증을 추가했습니다.
- 해당 버전의 ROM 변경은 `0x0A9C8F` 폴론 문단의 공백/닫기 명령 순서에 한정됩니다.
  번역 문장·글꼴·대사 분기 코드는 바꾸지 않았습니다.
- 제공된 PC 슬롯 1을 Snes9x Libretro로 재생해 대사 종료, 로코코로 이동,
  이후 주민 대화까지 창 테두리가 정상임을 확인했습니다. 정적 검증과 IPS 재적용 일치 검증도 통과했습니다.
- 새 ROM에서는 **깨지기 전 슬롯 1**부터 재개하세요. 깨진 뒤 저장한 상태는 손상된 그래픽 메모리도 포함합니다.
- 사용자 에뮬레이터에서의 재확인과 다른 장면의 진행 검증은 남아 있습니다.

## 적용 방법

1. 합법적으로 소유한 무헤더 영문판 `Robotrek (USA)` ROM을 준비합니다.
2. 사용하는 IPS 패처 또는 에뮬레이터의 IPS 기능으로
   `robotrek-korean-v0.1.17-alpha.ips`를 원본에 적용합니다.
