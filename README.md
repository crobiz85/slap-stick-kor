...
- 타이틀 로고와 저작권 표시는 이번 번역 범위에 포함하지 않았습니다.
  기존 저장 파일은 변경하지 않으며, GitHub 공개본은 `v0.1.12-alpha`를 유지합니다.

## v0.1.14-alpha 로컬 테스트 변경 (미공개)
...
- `v0.1.13`의 테두리 손상 수정은 포함하며, GitHub 공개본은 `v0.1.12-alpha`를 유지합니다.

## v0.1.13-alpha 로컬 테스트 변경 (미공개)
...
- 현재 ROM 변경은 `0x0A9C8F` 폴론 문단의 공백/닫기 명령 순서에 한정됩니다.
...
- 사용자 에뮬레이터에서의 재확인과 다른 장면의 진행 검증은 남아 있습니다.
  GitHub 공개본과 `patches/`의 `v0.1.12-alpha`는 교체하지 않았습니다.
