...
4. 한국어 글리프 위치와 텍스트 길이 제약을 검증합니다. 현재 초안 17개 모두 원문 슬롯을
   초과하므로, `translation/relocation-plan.tsv`를 바탕으로 문자열 컨테이너와 포인터 갱신 방식을 먼저 확정해야 합니다.
...
현재 17개 초안은 모두 원문 슬롯을 초과하며, 특히 `0x18000-0x27FFF` 구간의 인라인 레코드는
컨테이너 시작점 또는 이벤트 진입점 분석 없이 이동해서는 안 된다. 다음 구현 단계는 이 컨테이너의
65816 진입 참조를 에뮬레이터 트레이스 또는 디스어셈블 결과로 확인하는 것이다.
