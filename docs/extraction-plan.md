...
- 대화 출력 루틴이 사용하는 `0x82xx` 페이지와 게임 메뉴용 `0x83xx` 페이지의 일본어 미사용
  슬롯을 한글 글리프 영역으로 잡아 초안과 게임 화면 문구에 필요한 252자를
  `translation/korean-glyph-map.tsv`에 기록했다. `0x82xx` 글리프는 메뉴 페이지에 복사하고,
  0x82 페이지가 가득 찬 글리프는 0x83 페이지 오버플로 슬롯에 배치한다.
...
- 공개 기술 메모에서 확인한 `02 1D [주소]` 대사 인덱스와 동일 은행 FF 여유 공간을 이용해
  대사 0058~0063을 재배치하고, 메뉴 0001~0020·아이템 설명 0021~0035와 게임 화면 공통 문구 11개는
  원래 슬롯에 들어가는 프리뷰 문구로 바꿔 `0x82xx` 대화용 및 `0x83xx` 메뉴용 한글 폰트와 함께
