...
- 추출 스크립트에서 사용되지 않은 `80xx`·`81xx` 코드 207개를 한글 글리프에 할당해
  `translation/korean-glyph-map.tsv`에 기록한다. 일본어 스크립트에서 관측된 코드는 덮어쓰지 않는다.
...
4. 한국어 글리프 위치와 텍스트 길이 제약을 자동 검증합니다. 메인 대사 0058~0063과
   슬롯에 들어가는 메뉴 0001~0020과 아이템 설명 0021~0035는 프리뷰 패치에 반영했습니다.
