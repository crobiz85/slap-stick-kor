...
- 공개 기술 메모에서 확인한 `02 1D [주소]` 대사 인덱스와 동일 은행 FF 여유 공간을 이용해
  ...
  원래 슬롯에 들어가는 프리뷰 문구로 바꿔 `0x80xx`·`0x81xx` 한글 폰트와 함께
  `patches/slap-stick-kor-preview.bps` 및 `.ips`에 삽입함
- 상태/설정 화면은 일반 대사와 분리된 `Other Menus` 원시 블록으로 확인했으며, 장비 없음·설정·확인·취소
  고정 슬롯 5개를 `translation/korean-status-menu.tsv`로 검증해 패치함. 글꼴은 타일 경계에서 획이
  붙거나 잘리지 않도록 1픽셀 안전 여백을 둔 13px 굵은 글꼴로 다시 생성함.
...
4. 한국어 글리프 위치와 텍스트 길이 제약을 자동 검증합니다. 메인 대사 0058~0063,
   상태/설정 고정 문구 5개, C0 대사 30개와 슬롯에 들어가는 메뉴 0001~0020·아이템 설명
