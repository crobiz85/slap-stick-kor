...
4. 한국어 글리프 위치와 텍스트 길이 제약을 검증합니다. 메인 대사 0058~0063은 이 작업을 완료했고,
   초반 메뉴·이벤트 11개는 `translation/relocation-plan.tsv`를 바탕으로 컨테이너 진입점을 추가 확인해야 합니다.
5. 남은 11개를 재배치하고 65816 포인터·은행 매핑을 갱신한 뒤 프리뷰 패치에 통합합니다.
...
남기고, 0059~0063은 `0x5FEB3`부터 재배치했다. 반면 `0x18000-0x27FFF` 구간의 초반 메뉴·이벤트
11개는 컨테이너 시작점 또는 이벤트 진입점 분석 없이 이동하지 않고 원본 그대로 보류했다. 다음
구현 단계는 이 컨테이너의 65816 진입 참조를 에뮬레이터 트레이스 또는 디스어셈블 결과로 확인하는
것이다.
