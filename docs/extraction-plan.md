...
- 공개 기술 메모에서 확인한 `02 1D [주소]` 대사 인덱스와 동일 은행 FF 여유 공간을 이용해
  대사 0058~0063을 재배치하고, 초반 raw 메뉴 0002~0012는 원래 슬롯에 들어가는 압축형
  프리뷰 문구로 바꿔 미사용 0x82xx 한글 폰트와 함께 `patches/slap-stick-kor-preview.bps`
  및 `.ips`에 삽입함
...
4. 한국어 글리프 위치와 텍스트 길이 제약을 검증합니다. 메인 대사 0058~0063은 자연스러운
   원고 재배치까지 완료했고, 초반 메뉴·이벤트 11개는 슬롯에 맞춘 압축형 프리뷰를 먼저 적용했으므로
   이후 컨테이너 진입점이 확인되면 `translation/korean-draft.tsv`의 자연스러운 문구로 교체합니다.
...
11개는 진입 포인터가 확인되지 않아 우선 원래 슬롯 안의 압축형 한국어 프리뷰 문구를 적용했고,
